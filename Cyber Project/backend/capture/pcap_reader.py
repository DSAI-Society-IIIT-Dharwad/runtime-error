...
class PcapFileReader:
            ...
            with PcapReader(str(self.pcap_path)) as pcap:
                ...
                for packet in pcap:
                    ...
                    if hasattr(packet, 'time'):
                        # Convert EDecimal/float to timestamp - handle both types
                        try:
                    ...
                    if len(chunk) >= chunk_size:
                        ...
                        # Progress update
                        if self.processed_packets % 1000 == 0:
                            logger.info(f"Processed {self.processed_packets} packets")
                ...
                # Final flow flush - force flush ALL remaining flows
                logger.info("Flushing all remaining flows...")
                await self.processor._flush_flows(force_all=True)
    ...
    async def _send_chunk(self, chunk: List[Dict[str, Any]]):
        ...
        if self.processor.callback and chunk:
            try:
                # Group packets into flows for better processing
                flows_data = []
                for packet_data in chunk:
                    if packet_data.get('type') == 'flow':
                        flows_data.append(packet_data)
        ...
        try:
            ...
            with PcapReader(str(self.pcap_path)) as pcap:
                for i, packet in enumerate(pcap):
                    ...
                    if hasattr(packet, 'time'):
                        try:
                            last_timestamp = datetime.fromtimestamp(float(packet.time))
                        except (ValueError, TypeError):
                            pass
                    packet_count += 1
