...
class PacketProcessor:
    ...
    def process_packet(self, packet) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            if now - self.last_flush > self.flow_window:
                asyncio.create_task(self._flush_flows())
    ...
    async def _flush_flows(self):
        ...
        for key, flow in self.flows.items():
            if flow['last_time'] and (now - flow['last_time'] > self.flow_window):
                # Parse flow key
                parts = key.rsplit('-', 1)
                if len(parts) == 2:
...
class LiveSniffer:
    ...
    def __init__(self, interface: str, callback: Optional[Callable] = None):
        ...
        if interface not in get_if_list():
            logger.warning(f"Interface {interface} not found. Available: {get_if_list()}")
